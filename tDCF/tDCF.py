import sys
import numpy as np
import tDCF.metrics as em
import matplotlib.pyplot as plt



def main(cm_score, asv_score):

    

    # Replace CM scores with your own scores or provide score file as the first argument.
    #cm_score_file = 't-DCF/scores/cm_dev.txt'
    # Replace ASV scores with organizers' scores or provide score file as the second argument.
    #asv_score_file = 't-DCF/scores/asv_dev.txt'

    if (cm_score) :
        cm_score_file = cm_score
    if (asv_score) :
        asv_score_file = asv_score


    """ if len(args) > 1:
        if len(args) != 3:
            print('USAGE: python tDCF <CM_SCOREFILE> <ASV_SCOREFILE>')
            exit()
        else:
